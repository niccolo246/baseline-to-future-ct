...
class MultiScalePatchDiscriminator(nn.Sequential):
    """
    Multi-scale Patch-GAN discriminator for conditional adversarial models:
...
class PatchDiscriminator(nn.Sequential):
    """
    Patch-GAN discriminator for conditional adversarial models:
    High-Resolution Image Synthesis and Semantic Manipulation with Conditional GANs
    Ting-Chun Wang1, Ming-Yu Liu1, Jun-Yan Zhu2, Andrew Tao1, Jan Kautz1, Bryan Catanzaro (1)
    (1) NVIDIA Corporation, 2UC Berkeley
    In CVPR 2018.
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        Args:
            x: input tensor
            feature-matching loss (regulariser loss) on the discriminators as well.
        Returns:
